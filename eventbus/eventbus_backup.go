...
type EventBusOld[T TopicType] struct {
...
func NewEventBusOld[T TopicType](pctx context.Context, duration time.Duration) *EventBusOld[T] {
...
func (b *EventBusOld[T]) cleanupOldMessages() {
...
func (b *EventBusOld[T]) Subscribe(topic Topic[T], cap int) chan Event {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *EventBusOld[T]) Unsubscribe(topic Topic[T], ch chan Event) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *EventBusOld[T]) Publish(topic Topic[T], event Event) {
	b.mu.RLock()
